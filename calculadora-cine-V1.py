...
# Estilo CSS optimizado: fuentes, botones y reducción de interlineado
st.markdown("""
    <style>
    html, body, [class*="st-"] {
    ...
    /* Botón Nuevo cálculo (Azul suave) */
    ...
    /* Reducir interlineado en la lista de acumulados */
    .acumulado-text {
        margin-bottom: -15px;
        font-size: 0.9em;
    }
...
# --- EXTRAS (Corregido error visual y lógica de suma) ---
with st.expander("Añadir Horas Extras / Festivas"):
    col_qty, col_mult = st.columns(2)
    e_qty = col_qty.number_input("¿Cuántas?", min_value=0.0, step=1.0, format="%.f", key="input_qty")
    e_mult = col_mult.number_input("Factor (ej. 1,5)", min_value=1.0, value=1.5, step=0.1, key="input_mult")
    ...
    if st.session_state.extras_lista:
        ...
        for i, ex in enumerate(st.session_state.extras_lista):
            ...
            c_ex.markdown(f"<p class='acumulado-text'>⚡️ {ex['desc']}</p>", unsafe_allow_html=True)
            if c_del.button("x", key=f"del_ex_{i}"):
                st.session_state.extras_lista.pop(i)
                st.rerun()

# --- DIETAS (Corregido error visual) ---
with st.expander("Añadir Dietas"):
    c1, c2 = st.columns(2)
    if c1.button("🌯 Media comida"): st.session_state.dietas["comida"] += 1; st.rerun()
    if c2.button("🍽 Media cena"): st.session_state.dietas["cena"] += 1; st.rerun()
    if c1.button("🚍 Sin pernocta"): st.session_state.dietas["sin"] += 1; st.rerun()
    if c2.button("💤 Con pernocta"): st.session_state.dietas["con"] += 1; st.rerun()
...
if st.button("Calcular total", type="primary", use_container_width=True):
    ...
    # 2. Extras (Suma de todos los netos guardados en la lista)
...
# Espaciado para el botón de reinicio
